...
MUTATION_RATE = 0.05
#
# class say_hello(request):
#     queryset=Section.objects.filter(course__course_number=74)
#     render(request,'hello.html',{'name':'Karrtik','tags':queryset})
class Data:
    def __init__(self):
        ...
        for sec in Section.objects.all():
            for scd in schedule:
                if scd.section == sec.section_id:
                    ...
                    t.department = str(sec.department)
...
def faculty_timetabel(request):
    return render(request, 'demo.html')
def faculty_view(request):
    faculty = (request.GET['professor'])
    return render(request, 'demo2.html', {'timetable':TimeTableModel.objects.filter(instructor=faculty)})